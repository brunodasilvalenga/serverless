...
module.exports = {
  compileMethods() {
    ...
    this.validated.events.forEach(event => {
      ...
      const lambdaVersionLogicalId = (() => {
        const functionObject = this.serverless.service.functions[event.functionName];
        if (!functionObject.provisionedConcurrency) return null;
        return this.provider.naming.getLambdaProvisionedConcurrencyVersionLogicalId(
          event.functionName
        );
      })();
      ...
      _.merge(
        ...
        this.getMethodIntegration(event.http, { lambdaLogicalId, lambdaVersionLogicalId }),
